...
import org.ftp4che.ResultFactory;
 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class FTPConnectionImpl extends SuperFTPConnection implements FTPConnection {
    
    Logger log = Logger.getLogger(FTPConnectionImpl.class.getName());
    ReplyListener listener = null;
    
    public FTPConnectionImpl()
    {
        super();
    }
    ...
    public Result[] sendCommand(Command cmd,boolean fetchResult) throws IOException{
        List commands = cmd.getTextCommands();
        Result []results = new Result[commands.size()];
        for(int i=0; i < commands.size(); i++)
        {
            controlBuffer.clear();
            log.debug("Sending command: " + commands.get(i));
        	controlBuffer.put((String)commands.get(i));
        	controlBuffer.flip();
